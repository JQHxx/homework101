...
- (void)hideAnimated:(BOOL)animated withCompletionBlock:(void(^)())completion {
    if ([self.toolDelegate respondsToSelector:@selector(popToolCloseAction)]) {
        [self.toolDelegate popToolCloseAction];
    }
...
- (instancetype)init {
    ...
    if (self) {
        [self setImage:[UIImage drawImageWithName:@"popup_close" size:CGSizeMake(15, 15)] forState:UIControlStateNormal];
